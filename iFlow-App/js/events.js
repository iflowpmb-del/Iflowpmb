import {
    openAdjustCapitalModal, openConfirmModal, openEditStockModal,
    showClientHistoryModal, switchTab, switchSubTab, toggleTradeInDetails, showModal,
    openAddClientModal, showSaleDetailModal, formatCurrency, openEditClientModal,
    openEditDebtModal, openEditFixedExpenseModal, openExecutePaymentModal, updateSaleBalance,
    openExecuteDailyExpenseModal, openAddStockModal, openChangePasswordModal, openNoteModal,
    showItemDetailsModal, renderAddStockForm, renderSalesAnalysis, renderTradeInAttributes
} from './ui.js';
import { appState, setState, WALLET_CONFIG } from './state.js';
import { addData, deleteFromDb, updateData, setData, runBatch } from './api.js';
import { serverTimestamp, doc, collection, Timestamp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
import { signOut, updatePassword, EmailAuthProvider, reauthenticateWithCredential } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";
...
function handleSettleProviderDebt(debtId) {
    const message = "Esta acción eliminará la deuda de la lista. Recuerda ajustar manualmente el saldo de tus billeteras para reflejar el pago. ¿Deseas continuar?";
...
function handleSettleClientDebt(saleId, balance) {
    const message = `Vas a saldar una deuda de ${formatCurrency(balance, 'USD')}. La deuda total se reducirá automáticamente. Recuerda agregar este ingreso manualmente a la billetera correspondiente (ej. Dólares, Efectivo). ¿Continuar?`;
    ...
    openConfirmModal(message, async () => {
        try {
            ...
            await logCapitalState(`Cobro deuda de ${appState.sales.find(s => s.id === saleId)?.customerName || 'cliente'}`);
...
function handleWalletSelection(selectedButton) {
    const walletSelector = selectedButton.closest('#payment-wallet-selector, #daily-expense-wallet-selector');
    ...
    const confirmationContainer = selectedButton.closest('.modal-content').querySelector('.payment-confirmation-actions');
...
async function handleFinalPaymentConfirmation(confirmButton) {
    ...
    try {
        await runBatch(async (batch, db, userId) => {
            ...
            const expenseAmountInCurrency = walletConfig.currency === 'ARS' ? expense.amount * exchangeRate : expense.amount;
...
async function handleAnnulSale(saleId) {
    ...
    openConfirmModal(message, async () => {
        try {
            ...
            showModal("Venta anulada con éxito. Los productos han sido devueltos al stock. Recuerda ajustar el capital manually.", "Éxito");
...
async function saveStockItem(form, docId = null) {
    ...
    try {
        ...
        const stockItemData = {
            category: form.querySelector('[id$="-category-reg"], [id$="-category-modal"], [id$="-stock-category"]').value,
            model: form.querySelector('[id$="-model-reg"], [id$="-model-modal"], [id$="-stock-model"]').value.trim(),
            serialNumber: form.querySelector('[id$="-serial-reg"], [id$="-serial-modal"], [id$="-stock-serial"]').value.trim().toUpperCase(),
            phoneCost: parseFloat(form.querySelector('[id$="-cost-reg"], [id$="-cost-modal"], [id$="-stock-cost"]').value) || 0,
            suggestedSalePrice: parseFloat(form.querySelector('[id$="-price-reg"], [id$="-price-modal"], [id$="-stock-price"]').value) || 0,
            quantity: parseInt(form.querySelector('[id$="-quantity-reg"], [id$="-quantity-modal"], [id$="-stock-quantity"]').value, 10) || 1,
            details: form.querySelector('[id$="-details-reg"], [id$="-details-modal"], [id$="-stock-details"]')?.value.trim() || '',
...
async function handleSaveSale() {
    ...
    try {
        ...
        const totalSalePrice = (sale.items || []).reduce((sum, item) => sum + (item.salePrice || 0), 0) - (sale.discount || 0);
        let totalPaid = 0;
        document.querySelectorAll('.payment-input').forEach(input => {
            const value = parseFloat(input.value) || 0;
            totalPaid += (input.dataset.payment === 'ars' || input.dataset.payment === 'mp') ? value / exchangeRate : value;
        });
        ...
        await runBatch(async (batch, db, userId) => {
            ...
            if (clientDebtAmount > 0) capitalUpdates.clientDebt = (appState.capital.clientDebt || 0) + clientDebtAmount;
        ...
        setState({ sale: { clientSearchTerm: '', stockSearchTerm: '', selectedClient: null, items: [], discount: 0, saleCosts: 0 } });
...
export function setupEventListeners() {
    document.body.addEventListener('submit', async (e) => {
        const form = e.target;
        if (form.closest('.modal-content') || ['login-form', 'register-form', 'sale-form', 'stock-form-register', 'client-form-register', 'fixed-expense-form-register', 'daily-expense-form-register', 'debt-form', 'add-attribute-form'].includes(form.id)) {
            e.preventDefault();
        } else {
            return;
        }
        ...
        try {
            switch (form.id) {
                ...
                case 'sale-form': await handleSaveSale(); break;
                case 'password-change-form': await handleChangePassword(form); break;
                case 'stock-form-register': await saveStockItem(form); break;
                case 'client-form-register': await handleAddClient(form); break;
                case 'fixed-expense-form-register': await handleAddFixedExpense(form); break;
                case 'daily-expense-form-register': handleAddDailyExpense(form); break;
                case 'debt-form': await handleAddDebt(form); break;
                case 'client-form-modal': await handleModalClient(form); break;
                case 'stock-form-modal': await handleModalStock(form); break;
                case 'adjust-capital-form': await handleAdjustCapital(form); break;
                case 'note-form': {
                    const button = document.querySelector(`button[type="submit"][form="${form.id}"]`);
                    setButtonLoading(button, true);
                    try {
                ...
                case 'edit-stock-form': await saveStockItem(form, form.dataset.id); break;
                case 'edit-client-form': await handleEditClient(form); break;
                case 'edit-debt-form': await handleEditDebt(form); break;
                case 'edit-fixed-expense-form': await handleEditFixedExpense(form); break;
                case 'add-attribute-form': {
                    ...
                    const options = form.querySelector('#new-attribute-options').value.split(',').map(s => s.trim()).filter(Boolean);
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        const element = target.closest('button, a, div[data-client], div[data-stock], div[data-category-id], i#edit-business-name-icon');
        ...
        if (target.closest('.client-result-item')) {
            setState({ sale: { selectedClient: JSON.parse(target.closest('.client-result-item').dataset.client), clientSearchTerm: '' } });
            return;
        }
        if (target.closest('.stock-result-item')) {
            const currentItems = appState.sale.items || [];
            const newItem = JSON.parse(target.closest('.stock-result-item').dataset.stock);
            setState({ sale: { items: [...currentItems, { ...newItem, salePrice: newItem.suggestedSalePrice || 0 }], stockSearchTerm: '' } });
            return;
        }
        ...
        if (!element) {
            ...
            if (!target.closest('#client-search-input-sale') && !target.closest('#client-search-results')) {
                document.getElementById('client-search-results')?.classList.add('hidden');
            }
            if (!target.closest('#stock-search-input-sale') && !target.closest('#stock-search-results-sale')) {
...
Allí encontrarás un enlace para administrar tu compra y ver tus facturas.`,
            ...
            'confirm-gumroad-redirect': async (button) => {
                ...
                // Se establece el estado a 'pendiente' para que el sistema espere la validación del webhook.
                await setData('profile', 'main', { subscriptionStatus: 'pending_payment_validation' }, true);
                setButtonLoading(button, false);
            },
            // --- FIN DE LA MODIFICACIÓN ---
            'toggle-sale-form-btn': () => document.getElementById('add-sale-form-container')?.classList.toggle('hidden'),
            'toggle-stock-form-btn': () => setState({ addStockForm: { isFormVisible: !appState.addStockForm.isFormVisible } }),
            'toggle-client-form-btn': () => document.getElementById('add-client-form-container')?.classList.toggle('hidden'),
            'toggle-debt-form-btn': () => document.getElementById('add-debt-form-container')?.classList.toggle('hidden'),
            'adjust-capital-btn': () => openAdjustCapitalModal(appState),
            'add-client-from-sale-btn': () => openAddClientModal(),
            'remove-selected-client-btn': () => setState({ sale: { selectedClient: null, clientSearchTerm: '' } }),
            'dashboard-apply-custom-filter-btn': () => {
                ...
                setState({ ui: { dashboard: { dashboardCustomStartDate: startDate, dashboardCustomEndDate: endDate } } });
            },
            'analysis-apply-custom-filter-btn': () => {
                ...
                setState({ ui: { analysis: { analysisCustomStartDate: startDate, analysisCustomEndDate: endDate } } });
            },
            ...
            'delete-category-btn': () => {
                ...
                if (isCategoryInUse) {
                    return showModal(`No se puede eliminar la categoría "${category.name}" porque está siendo utilizada por productos en el stock.`);
                }
                  
                openConfirmModal(`¿Seguro que quieres eliminar la categoría "${category.name}"?`, async () => {
        ...
        const classActionMap = {
            ...
            'category-manager-item': () => {
                if (dataset.categoryId) {
                    setState({ categoryManager: { selectedCategoryId: dataset.categoryId, isEditingCategoryName: false } });
                }
            },
            ...
            'remove-sale-item-btn': () => {
                const newItems = appState.sale.items.filter((_, index) => index !== parseInt(dataset.index, 10));
            ...
            'delete-client-btn': () => {
                if (appState.sales?.some(s => s.clientId === dataset.id)) {
                    return showModal(`No se puede eliminar a <strong>${dataset.name}</strong> porque tiene ventas asociadas.`);
                }
                openConfirmModal(`¿Seguro que quieres eliminar a ${dataset.name}?`, () => deleteFromDb('clients', dataset.id));
            },
            ...
            'delete-fixed-expense-btn': () => handleManualDelete('fixed_expenses', dataset.id, 'el gasto fijo'),
            ...
            'delete-daily-expense-btn': () => handleManualDelete('daily_expenses', dataset.id, 'el gasto'),
            'settle-our-debt-btn': () => handleSettleProviderDebt(dataset.debtId),
            'settle-client-debt-btn': () => handleSettleClientDebt(dataset.saleId, parseFloat(dataset.balance)),
            ...
            'delete-note-btn': () => {
                openConfirmModal('¿Estás seguro de que quieres eliminar esta nota? Esta acción no se puede deshacer.', () => deleteFromDb('notes', dataset.noteId));
            },
    ...
    document.body.addEventListener('input', (e) => {
        ...
        const actionMap = {
            'client-search-input-sale': () => setState({ sale: { clientSearchTerm: target.value, selectedClient: null } }),
    ...
    document.body.addEventListener('change', (e) => {
        ...
        if (target.id === 'stock-category-modal' || target.id === 'edit-stock-category') {
            ...
            if (container) {
                container.innerHTML = selectedCategory?.attributes?.map(attr => generateAttributeInputHTML(attr)).join('') || '';
            }
        }
        ...
        if (target.matches('.payment-input, #trade-in-value, #sale-discount-input, .sale-item-price, #sale-costs-input')) {
            if (target.id === 'sale-discount-input') setState({ sale: { discount: parseFloat(target.value) || 0 } });
            if (target.id === 'sale-costs-input') setState({ sale: { saleCosts: parseFloat(target.value) || 0 } });
