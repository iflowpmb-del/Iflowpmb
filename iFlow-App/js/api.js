...
import { collection, onSnapshot, query, doc, deleteDoc, addDoc, serverTimestamp, setDoc, updateDoc, writeBatch, Timestamp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
import { setState, addFirebaseListener, clearFirebaseListeners, appState, DEFAULT_CATEGORIES } from './state.js';
...
export function loadAllData(userId) {
    ...
    const collectionsToLoad = {
        profile: { type: 'doc', path: `users/${userId}/profile/main`, default: { businessName: 'Mi Negocio', subscriptionStatus: 'none' } },
        capital: { type: 'doc', path: `users/${userId}/capital/summary`, default: { ars: 0, usd: 0, usdt: 0, mp: 0, clientDebt: 0, debt: 0 } },
        stock: { type: 'collection', path: `users/${userId}/stock` },
        sales: { type: 'collection', path: `users/${userId}/sales`, sorter: (a, b) => (b.soldAt?.toMillis() || 0) - (a.soldAt?.toMillis() || 0) },
        ...
        dailyExpenses: { type: 'collection', path: `users/${userId}/daily_expenses`, sorter: (a, b) => new Date(b.date) - new Date(a.date) },
        providers: { type: 'collection', path: 'providers' },
        notes: { type: 'collection', path: `users/${userId}/notes`, sorter: (a, b) => (b.updatedAt?.toMillis() || 0) - (a.updatedAt?.toMillis() || 0) },
        capitalHistory: { type: 'collection', path: `users/${userId}/capital_history`, sorter: (a, b) => (a.timestamp?.toMillis() || 0) - (b.timestamp?.toMillis() || 0) },
    };

    let initialLoadsPending = Object.keys(collectionsToLoad).length;
    let initialLoadFlags = Object.keys(collectionsToLoad).reduce((acc, key) => ({ ...acc, [key]: true }), {});
    ...
    for (const [key, config] of Object.entries(collectionsToLoad)) {
        ...
        if (key === 'categories') {
            ...
            const categoriesUnsubscribe = onSnapshot(query(categoriesCollectionRef), async (snapshot) => {
        ...
        const queryRef = config.type === 'doc'
            ? doc(db, config.path)
            : query(collection(db, config.path));
        ...
        const unsubscribe = onSnapshot(queryRef, (snapshot) => {
            ...
            if (config.type === 'doc') {
                ...
                if (key === 'profile' && snapshot.exists() && !data.subscriptionStatus) {
                    console.warn(`[API] Usuario antiguo detectado (${userId}). Actualizando perfil para iniciar período de prueba.`);
                    
                    // <<<--- INICIO DE LA MODIFICACIÓN ---
                    // Se elimina la creación de la fecha de finalización del trial.
                    // const trialEndDate = new Date();
                    // trialEndDate.setDate(trialEndDate.getDate() + 7);
                    const updatedFields = {
                        subscriptionStatus: 'trial',
                        // Ya no se establece una fecha de finalización para la prueba.
