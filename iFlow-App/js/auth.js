...
export async function handleRegistration(email, password) {
    try {
        ...
        batch.set(profileRef, {
            ...
            exchangeRate: 1000,
            email: email,
            subscriptionStatus: 'pending_trial_setup', // Estado inicial correcto
        });
        ...
        await batch.commit();
        console.log('[AUTH] Perfil base creado correctamente con estado "pending_trial_setup".');

    } catch (error) {
        let friendlyMessage = "Ocurrió un error durante el registro.";
        if (error.code === 'auth/email-already-in-use') {
            friendlyMessage = "Este correo electrónico ya está en uso. Por favor, inicia sesión o utiliza otro correo.";
